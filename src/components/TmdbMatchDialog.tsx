...
function extractDirectors(tmdb: TmdbMovieDetail): string[] {
...
function extractCrew(tmdb: TmdbMovieDetail): CrewUpdateInfo[] {
  ...
  return tmdb.credits.crew
    ...
    .map((c) => ({ name: c.name, job: c.job ?? null }));
}

function extractProducers(tmdb: TmdbMovieDetail): string[] {
  if (!tmdb.credits?.crew) return [];
  return tmdb.credits.crew
    .filter(
      (c) =>
...
function buildReviewFields(
  ...
  fields.push({
    ...
    tmdbDisplay: formatList(extractDirectors(tmdb)),
  ...
  fields.push({
    ...
    currentDisplay: formatCast(
      current.cast.map((c) => ({ name: c.name, role: c.role ?? null }))
    ),
  ...
  fields.push({
    ...
    currentDisplay: formatCrew(
      current.crew.map((c) => ({ name: c.name, job: c.job ?? null }))
    ),
  ...
  fields.push({
    ...
    tmdbDisplay: formatList(extractProducers(tmdb)),
