...
pub async fn get_person_images(
    ...
    token: &str,
    tmdb_id: i64,
) -> Result<Vec<TmdbPersonImage>, String> {
    let url = format!("{BASE_URL}/person/{tmdb_id}/images");
    let resp = client
        .get(&url)
        .bearer_auth(token)
        .send()
    ...
    let parsed: TmdbPersonImagesResponse = resp
        .json()
        .await
        .map_err(|e| format!("Failed to parse TMDB person images: {e}"))?;
    Ok(parsed.profiles)
}
